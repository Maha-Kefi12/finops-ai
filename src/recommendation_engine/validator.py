...
def _validate_ec2_rightsize(rec: Dict, node: Dict, graph_data: dict) -> Dict:
    ...
    is_idle = (
        cpu_util < VALIDATION_THRESHOLDS["ec2_idle_cpu_p95"] and
        (network_in + network_out) < VALIDATION_THRESHOLDS["ec2_idle_network_mbps"]
    )
    
    is_underutilized = cpu_util < 30.0  # Less than 30% CPU
    ...
    # Validate savings estimate
    current_cost = node.get("cost_monthly", 0)
    estimated_savings = rec.get("total_estimated_savings", 0)
    
    if estimated_savings < VALIDATION_THRESHOLDS["min_monthly_savings"]:
        return {
            "is_valid": False,
            "confidence": 0.0,
            "notes": f"Rejected: Estimated savings ${estimated_savings:.2f}/mo < minimum ${VALIDATION_THRESHOLDS['min_monthly_savings']}/mo"
        }
    ...
    if current_cost > 0 and estimated_savings > current_cost:
        return {
            "is_valid": False,
            "confidence": 0.0,
            "notes": f"Rejected: Estimated savings ${estimated_savings:.2f} exceeds current cost ${current_cost:.2f}"
        }
...
def _validate_cache_optimization(rec: Dict, node: Dict, graph_data: dict) -> Dict:
    ...
    if is_oversized:
        ...
        return {
            "is_valid": False,
            "confidence": 0.0,
            "notes": f"Rejected: Cache metrics do not indicate oversizing"
        }
...
def _validate_nat_optimization(rec: Dict, node: Dict, graph_data: dict) -> Dict:
    ...
    is_idle = bytes_per_hour < VALIDATION_THRESHOLDS["nat_idle_bytes_per_hour"]
...
def _validate_lambda_optimization(rec: Dict, node: Dict, graph_data: dict) -> Dict:
    ...
    # Check if memory is in sweet spot range
    in_sweet_spot = (
        VALIDATION_THRESHOLDS["lambda_memory_sweet_spot_min"] <= current_memory <= 
        VALIDATION_THRESHOLDS["lambda_memory_sweet_spot_max"]
    )
    
    if not in_sweet_spot and current_memory > 0:
        confidence = 0.70
...
def _validate_generic(rec: Dict, node: Dict, graph_data: dict) -> Dict:
    ...
    if current_cost > 0 and estimated_savings > current_cost * 0.9:
        return {
            "is_valid": False,
            "confidence": 0.0,
            "notes": f"Rejected: Savings ${estimated_savings:.2f} exceeds 90% of current cost ${current_cost:.2f}"
        }
    
    # If basic checks pass, accept with medium confidence
    return {
        "is_valid": True,
        "confidence": 0.60,
        "notes": "Validated: Passed basic sanity checks (no specific rule available)"
    }
