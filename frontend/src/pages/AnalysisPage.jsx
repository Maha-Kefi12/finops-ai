...
import {
    ...
    CircleDot, TrendingDown, Flame, HardDrive, Workflow, BookOpen
} from 'lucide-react'
...
export default function AnalysisPage() {
                    ...
                    {recResult && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-4">
                                {(recResult.recommendations || []).map((card, idx) => (
                                    <FullRecommendationCard
                                        key={idx}
                                        card={card}
                                        index={idx}
                                        isExpanded={!!expandedCards[idx]}
                                        onToggle={() => setExpandedCards(prev => ({ ...prev, [idx]: !prev[idx] }))}
                                    />
                                ))}
                            </div>

                            {/* Context package summary */}
                            {recResult.context_package && (
                                <div className="card p-5">
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                        {[
                                            { label: 'Anti-Patterns', value: recResult.context_package.anti_patterns?.length || 0, icon: AlertTriangle, color: 'amber' },
                                            { label: 'Risks Identified', value: recResult.context_package.risks?.length || 0, icon: Shield, color: 'red' },
                                            { label: 'Anomalies', value: recResult.context_package.anomalies?.length || 0, icon: Activity, color: 'purple' },
                                            { label: 'Critical Deps', value: recResult.context_package.critical_dependencies?.length || 0, icon: GitBranch, color: 'blue' },
    ...
    function setTab(key) {
        ...
        if (key === 'agents' && !result && !loading) { /* user clicks Run manually */ }
    }
}
